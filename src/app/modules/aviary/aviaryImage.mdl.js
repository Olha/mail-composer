		...
		function ngAviaryDirective(ngAviary) {
			return {
				...
				link: function (scope, element, attrs) {
					var targetImage;
					setTimeout(function () {
						targetImage =  window.document.querySelector(scope.targetSelector);
					}, 0);
					...
					function launchEditor() {
						featherEditor.launch({
							image: targetImage,
							url: element.attr('target-src') || targetImage.src
						});
					...
					//additional functional for close aviary popup
					scope.$on('closeAviaryPopup', function (event, hndl) {
						featherEditor.close();
					});
				}
			};
